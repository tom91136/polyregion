...
#pragma region requires: 12 3.000000 42 5.000000 7.000000 54 1
...
int main() {
  ...
  int sum = __polyregion_offload_f1__([CHECK_CAPTURE]() { return static_cast<const A &>(value).x + static_cast<const B &>(value).x; });
  int nullPreserved = __polyregion_offload_f1__([]() {
    C *value = nullptr;
    return static_cast<const B *>(value) == nullptr ? 1 : 0;
  });
  printf("%d %f %d %f %f %d %d",        //
         static_cast<A &>(result).x,    // From A's Base
         result.y,                      // From A's y
         static_cast<B &>(result).x,    // From B's Base
         result.z,                      // From B's z
         result.w, sum, nullPreserved); // From C's w
  return 0;
}
