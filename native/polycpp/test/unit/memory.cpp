...
struct Fixture {
  ...
  using QueryPtr = std::function<polyregion::polystl::PtrQuery(const void *)>;
  ...
  polyregion::polystl::SynchronisedAllocation<QueryPtr, AllocateRemote, ReadRemote, WriteRemote, FreeRemote> allocation;
      ...
      : allocation(
            [&](const void *ptr) -> polyregion::polystl::PtrQuery {
              ...
              if (const auto it = localAllocations.find(localPtr); it != localAllocations.end()) {
                return polyregion::polystl::PtrQuery{it->second, 0};
              }
              for (auto [p, size] : localAllocations) {
                if (localPtr >= static_cast<uintptr_t>(p) && localPtr < static_cast<uintptr_t>(p) + size) {
                  return polyregion::polystl::PtrQuery{size, localPtr - static_cast<uintptr_t>(p)};
                }
              }
