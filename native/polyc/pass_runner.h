...
struct POLYREGION_EXPORT PassRunner {
  ...
  virtual std::optional<uint32_t> packageAbiVersion() const = 0;
  virtual Vector<uint8_t> runPackage(std::string_view operation, const Vector<uint8_t> &request, String &error) = 0;
