...
template <typename ELFT> static auto extractGeneric(const llvm::object::ELFObjectFile<ELFT> &obj) {
  ...
  if (auto sections = file.sections(); auto e = sections.takeError())
    POLYINVOKE_FATAL(PREFIX, "Cannot read ELF sections: %s", toString(std::move(e)).c_str());
  else {
    for (const auto s : *sections) {
      ...
      for (ELFT_Note note : file.notes(s, Err)) {
        ...
        try {
          ...
          POLYINVOKE_FATAL(PREFIX, "Illegal AMDGPU METADATA in .note section (%s)", e.what());
        }
      }
    }
    POLYINVOKE_FATAL(PREFIX, "ELF image does not contains AMDGPU METADATA in the .note section (sections=%zu)", sections->size());
  }
};
...
static details::SymbolArgOffsetTable extractSymbolArgOffsetTable(const std::string &image) {
  ...
  if (auto object = ObjectFile::createObjectFile(llvm::MemoryBufferRef(llvm::StringRef(image), "")); auto e = object.takeError()) {
    POLYINVOKE_FATAL(PREFIX, "Cannot load module: %s", toString(std::move(e)).c_str());
  } else {
    if (const ELFObjectFileBase *elfObj = llvm::dyn_cast<ELFObjectFileBase>(object->get())) {
      ...
      POLYINVOKE_FATAL(PREFIX, "Unrecognised ELF variant: %u", elfObj->getType());
    } else POLYINVOKE_FATAL(PREFIX, "Object image TypeID %d is not in the ELF range", object->get()->getType());
  }
}
    ...
    : queueSize(queueSize), hostAgent(hostAgent), agent(agent),
      store(
          ...
          [this](auto &&s) {
            ...
            if (validationResult != 0) POLYINVOKE_FATAL(PREFIX, "Cannot validate executable, code=%d", validationResult);
...
HsaDevice::~HsaDevice() { POLYINVOKE_TRACE(); }
...
void HsaDeviceQueue::enqueueInvokeAsync(const std::string &moduleName, const std::string &symbol, const std::vector<Type> &types,
  ...
  if (!kernargAddress && args.size() - 1 != 0) {
    POLYINVOKE_FATAL(PREFIX, "Kernarg address is NULL but we got %zu args to write", args.size() - 1);
  }
