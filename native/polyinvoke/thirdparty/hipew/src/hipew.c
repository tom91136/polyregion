...
#  define dynamic_library_open(path)         dlopen(path, RTLD_LAZY | RTLD_LOCAL)
...
#else
  const char *hip_paths[] = {"/lib64/libamdhip64.so.6", "/lib64/libamdhip64.so", "/opt/rocm/lib/libamdhip64.so", NULL};
  const char* hiprtc_paths[] = { NULL };
#endif
