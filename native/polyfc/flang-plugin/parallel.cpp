  ...
  return params ^
         fold_total(
             ...
             [&](const GPUParams &p) {
               ...
               // Define a local pointer for each reduction at offset = sum_{prev}(localSize * sizeof(prevType)).
               // The running offset is always materialised as a let so we can carry a Term::Any across iterations.
               Term::Any offset = letBind(body, "off", Expr::Alias(Term::IntS64Const(0)));
               std::vector<Named> localTargets;
               for (auto &r : reductions) {
                 // FIXME: assume 8 bytes per element until SingleVarReduction carries the primitive size; over-allocates for 4-byte types.
                 const Term::Any compSize = Term::IntS64Const(8);
