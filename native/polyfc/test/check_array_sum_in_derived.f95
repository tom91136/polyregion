...
program test
    ...
    allocate(foo%y(CHECK_SIZE))
    allocate(foo%z(CHECK_SIZE))
#endif

    foo%x = -1
    foo%y = [ (i, i = 1, CHECK_SIZE) ]
    foo%z = [ (i * 2, i = 1, CHECK_SIZE) ]
    do concurrent (i = 1:CHECK_SIZE)
        foo%x(i) = foo%y(i) + foo%z(i)
    end do

    if (all(foo%x == foo%y + foo%z)) then
        write(*, '(A)', advance = "no") "pass"
    else
        print*, "fail, array = ", foo%x
    end if
