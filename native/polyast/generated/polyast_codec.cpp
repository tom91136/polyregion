...
Type::Struct Type::struct_from_json(const json &j_) {
  ...
  for (const auto &v_ : j_.at(3)) {
    parents.emplace_back(sym_from_json(v_));
  }
...
json Type::struct_to_json(const Type::Struct &x_) {
  ...
  for (const auto &v_ : x_.parents) {
    parents.emplace_back(sym_to_json(v_));
  }
...
Expr::Unit0Const Expr::unit0const_from_json(const json &j_) { return {}; }

json Expr::unit0const_to_json(const Expr::Unit0Const &x_) { return json::array({}); }
...
Expr::Select Expr::select_from_json(const json &j_) {
  ...
  for (const auto &v_ : j_.at(0)) {
    init.emplace_back(named_from_json(v_));
  }
...
json Expr::select_to_json(const Expr::Select &x_) {
  ...
  for (const auto &v_ : x_.init) {
    init.emplace_back(named_to_json(v_));
  }
...
Expr::Invoke Expr::invoke_from_json(const json &j_) {
  ...
  for (const auto &v_ : j_.at(3)) {
    args.emplace_back(Expr::any_from_json(v_));
  }
  std::vector<Expr::Any> captures;
  for (const auto &v_ : j_.at(4)) {
    captures.emplace_back(Expr::any_from_json(v_));
  }
...
json Expr::invoke_to_json(const Expr::Invoke &x_) {
  ...
  std::vector<json> args;
  for (const auto &v_ : x_.args) {
    args.emplace_back(Expr::any_to_json(v_));
  }
  std::vector<json> captures;
  for (const auto &v_ : x_.captures) {
    captures.emplace_back(Expr::any_to_json(v_));
  }
  auto rtn = Type::any_to_json(x_.rtn);
...
json Expr::any_to_json(const Expr::Any &x_) {
  return x_.match_total([](const Expr::Float16Const &y_) -> json { return {0, Expr::float16const_to_json(y_)}; },
...
Stmt::Block Stmt::block_from_json(const json &j_) {
  ...
  for (const auto &v_ : j_.at(0)) {
    stmts.emplace_back(Stmt::any_from_json(v_));
  }
...
json Stmt::block_to_json(const Stmt::Block &x_) {
  ...
  for (const auto &v_ : x_.stmts) {
    stmts.emplace_back(Stmt::any_to_json(v_));
  }
...
Stmt::While Stmt::while_from_json(const json &j_) {
  ...
  for (const auto &v_ : j_.at(0)) {
    tests.emplace_back(Stmt::any_from_json(v_));
  }
...
json Stmt::while_to_json(const Stmt::While &x_) {
  ...
  for (const auto &v_ : x_.tests) {
    tests.emplace_back(Stmt::any_to_json(v_));
  }
...
json Stmt::any_to_json(const Stmt::Any &x_) {
  return x_.match_total([](const Stmt::Block &y_) -> json { return {0, Stmt::block_to_json(y_)}; },
...
InvokeSignature invokesignature_from_json(const json &j_) {
  ...
  for (const auto &v_ : j_.at(4)) {
    captures.emplace_back(Type::any_from_json(v_));
  }
  auto rtn = Type::any_from_json(j_.at(5));
...
json invokesignature_to_json(const InvokeSignature &x_) {
  ...
  std::vector<json> captures;
  for (const auto &v_ : x_.captures) {
    captures.emplace_back(Type::any_to_json(v_));
  }
  auto rtn = Type::any_to_json(x_.rtn);
  return json::array({name, tpeVars, receiver, args, captures, rtn});
}

FunctionAttr::Internal FunctionAttr::internal_from_json(const json &j_) { return {}; }

json FunctionAttr::internal_to_json(const FunctionAttr::Internal &x_) { return json::array({}); }

FunctionAttr::Exported FunctionAttr::exported_from_json(const json &j_) { return {}; }

json FunctionAttr::exported_to_json(const FunctionAttr::Exported &x_) { return json::array({}); }

FunctionAttr::FPRelaxed FunctionAttr::fprelaxed_from_json(const json &j_) { return {}; }

json FunctionAttr::fprelaxed_to_json(const FunctionAttr::FPRelaxed &x_) { return json::array({}); }

FunctionAttr::FPStrict FunctionAttr::fpstrict_from_json(const json &j_) { return {}; }

json FunctionAttr::fpstrict_to_json(const FunctionAttr::FPStrict &x_) { return json::array({}); }

FunctionAttr::Entry FunctionAttr::entry_from_json(const json &j_) { return {}; }

json FunctionAttr::entry_to_json(const FunctionAttr::Entry &x_) { return json::array({}); }
...
json FunctionAttr::any_to_json(const FunctionAttr::Any &x_) {
  return x_.match_total([](const FunctionAttr::Internal &y_) -> json { return {0, FunctionAttr::internal_to_json(y_)}; },
...
Function function_from_json(const json &j_) {
  ...
  for (const auto &v_ : j_.at(8)) {
    attrs.emplace(FunctionAttr::any_from_json(v_));
  }
...
json function_to_json(const Function &x_) {
  ...
  for (const auto &v_ : x_.attrs) {
    attrs.emplace_back(FunctionAttr::any_to_json(v_));
  }
...
json hashed_to_json(const json &x_) { return json::array({"647d2eb67e52e045fbf8d97386682159", x_}); }
} // namespace polyregion::polyast
