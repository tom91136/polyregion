...
std::string repr(const Type::Any &t) {
  return [&] {
    ...
    if (auto _x = t.get<Type::Struct>()) {
      return fmt::format("{}<{}>({}) <: {}", repr(_x->name), (_x->tpeVars | mk_string(","s)),
                         (_x->args | map([&](const Type::Any &__1) { return repr(__1); }) | mk_string(","s)),
                         (_x->parents | map([&](const Sym &__2) { return repr(__2); }) | mk_string(", "s)));
    }
    if (auto _x = t.get<Type::Ptr>()) {
      return fmt::format("{}{}{}", repr(_x->comp),
                         _x->length ^ map([&](const int32_t &__3) { return std::to_string(__3); }) ^
                             map([&](const std::string &l) { return fmt::format("[{}]", l); }) ^ get_or_else("*"s),
                         repr(_x->space));
    }
    ...
    if (auto _x = t.get<Type::Exec>()) {
      return fmt::format("<{}>({}) => {}", (_x->tpeVars | mk_string(","s)),
                         (_x->args | map([&](const Type::Any &__4) { return repr(__4); }) | mk_string(","s)), repr(_x->rtn));
    }
    if (auto _x = t.get<Type::Annotated>()) {
      return fmt::format("({}{}{})", repr(_x->tpe),
                         _x->pos ^ map([&](const SourcePosition &s) { return fmt::format("/* {} */", repr(s)); }) ^ get_or_else(""s),
                         _x->comment ^ map([&](const std::string &s) { return fmt::format("/* {} */", s); }) ^ get_or_else(""s));
    }
...
std::string repr(const Expr::Any &e) {
  return [&] {
    ...
    if (auto _x = e.get<Expr::Select>()) {
      return (_x->init | append(_x->last) | map([&](const Named &x) { return fmt::format("{}: {}", x.symbol, repr(x.tpe)); }) |
              reduce([&](const std::string &acc, const std::string &x) { return fmt::format("({}).{}", acc, x); })) ^
             get_or_else(""s);
    }
    ...
    if (auto _x = e.get<Expr::RefTo>()) {
      return fmt::format("({}).refTo[{}, {}]({})", repr(_x->lhs), repr(_x->comp), repr(_x->space),
                         _x->idx ^ map([&](const Expr::Any &__5) { return repr(__5); }) ^ get_or_else(""s));
    }
    ...
    if (auto _x = e.get<Expr::Invoke>()) {
      return fmt::format("{}{}<{}>({})[{}]: {}",
                         _x->receiver ^ map([&](const Expr::Any &r) { return fmt::format("{}.", repr(r)); }) ^ get_or_else(""s),
                         repr(_x->name), (_x->tpeArgs | map([&](const Type::Any &__6) { return repr(__6); }) | mk_string(","s)),
                         (_x->args | map([&](const Expr::Any &__7) { return repr(__7); }) | mk_string(", "s)),
                         (_x->captures | map([&](const Expr::Any &__8) { return repr(__8); }) | mk_string(", "s)), repr(_x->rtn));
    }
    if (auto _x = e.get<Expr::Annotated>()) {
      return fmt::format("({}{}{})", repr(_x->expr),
                         _x->pos ^ map([&](const SourcePosition &s) { return fmt::format("/* {} */", repr(s)); }) ^ get_or_else(""s),
                         _x->comment ^ map([&](const std::string &s) { return fmt::format("/* {} */", s); }) ^ get_or_else(""s));
    }
...
std::string repr(const Stmt::Any &stmt) {
  return [&] {
    if (auto _x = stmt.get<Stmt::Block>()) {
      return fmt::format("{}\n{}\n{}", "{"s,
                         (_x->stmts | map([&](const Stmt::Any &__9) { return repr(__9); }) | mk_string("\n"s)) ^ indent(2), "}"s);
    }
    if (auto _x = stmt.get<Stmt::Comment>()) {
      return fmt::format(" /* {} */", _x->value);
    }
    if (auto _x = stmt.get<Stmt::Var>()) {
      return fmt::format("var {}: {} = {}", _x->name.symbol, repr(_x->name.tpe),
                         _x->expr ^ map([&](const Expr::Any &__10) { return repr(__10); }) ^ get_or_else("_"s));
    }
    ...
    if (auto _x = stmt.get<Stmt::While>()) {
      return fmt::format("while({}{}{}){}\n{}\n{}", "{"s,
                         (_x->tests | map([&](const Stmt::Any &__11) { return repr(__11); }) | append(repr(_x->cond)) | mk_string(";"s)),
                         "}"s, "{"s, (_x->body | map([&](const Stmt::Any &__12) { return repr(__12); }) | mk_string("\n"s)) ^ indent(2),
                         "}"s);
    }
    if (auto _x = stmt.get<Stmt::ForRange>()) {
      return fmt::format("for({} = {}; {} < {}; {} += {}){}\n{}\n{}", repr(_x->induction), repr(_x->lbIncl), repr(_x->induction),
                         repr(_x->ubExcl), repr(_x->induction), repr(_x->step), "{"s,
                         (_x->body | map([&](const Stmt::Any &__13) { return repr(__13); }) | mk_string("\n"s)) ^ indent(2), "}"s);
    }
    ...
    if (auto _x = stmt.get<Stmt::Cond>()) {
      return fmt::format(
          "if({}) {}\n{}\n{}{}", repr(_x->cond), "{"s,
          (_x->trueBr | map([&](const Stmt::Any &__14) { return repr(__14); }) | mk_string("\n"s)) ^ indent(2), "}"s,
          (_x->falseBr.empty()
               ? ""s
               : fmt::format(" else {}\n{}\n{}", "{"s,
                             (_x->falseBr | map([&](const Stmt::Any &__15) { return repr(__15); }) | mk_string("\n"s)) ^ indent(2), "}"s)));
    }
    if (auto _x = stmt.get<Stmt::Annotated>()) {
      return fmt::format("{}{}{}", repr(_x->stmt),
                         _x->pos ^ map([&](const SourcePosition &s) { return fmt::format("/* {} */", repr(s)); }) ^ get_or_else(""s),
                         _x->comment ^ map([&](const std::string &s) { return fmt::format("/* {} */", s); }) ^ get_or_else(""s));
    }
...
std::string repr(const FunctionAttr::Any &a) {
  return [&] {
    if (a.is<FunctionAttr::Internal>()) {
      return "Internal"s;
    }
    if (a.is<FunctionAttr::Exported>()) {
      return "Exported"s;
    }
    if (a.is<FunctionAttr::FPRelaxed>()) {
      return "FPRelaxed"s;
    }
    if (a.is<FunctionAttr::FPStrict>()) {
      return "FPStrict"s;
    }
    if (a.is<FunctionAttr::Entry>()) {
      return "Entry"s;
    }
...
std::string repr(const Signature &f) {
  return fmt::format("def {}{}<{}>({}): {} /* mod={} term={} */",
                     ...
                     (f.tpeVars | mk_string(","s)), (f.args | map([&](const Type::Any &__16) { return repr(__16); }) | mk_string(", "s)),
                     repr(f.rtn), (f.moduleCaptures | map([&](const Type::Any &__17) { return repr(__17); }) | mk_string(","s)),
                     (f.termCaptures | map([&](const Type::Any &__18) { return repr(__18); }) | mk_string(","s)));
}
...
std::string repr(const Function &f) {
  return fmt::format(
      ...
      repr(f.rtn), (f.attrs | map([&](const FunctionAttr::Any &__19) { return repr(__19); }) | mk_string(","s)),
      (f.moduleCaptures | map([&](const Arg &__20) { return repr(__20); }) | mk_string(","s)),
      (f.termCaptures | map([&](const Arg &__21) { return repr(__21); }) | mk_string(","s)), "{"s,
      (f.body | map([&](const Stmt::Any &__22) { return repr(__22); }) | mk_string("\n"s)) ^ indent(2), "}"s);
}
...
std::string repr(const StructDef &s) {
  return fmt::format("class {}<{}>({}) <: {}", repr(s.name), (s.tpeVars | mk_string(","s)),
                     ...
                     (s.parents | map([&](const Sym &__23) { return repr(__23); }) | mk_string(", "s)));
}

std::string repr(const Program &s) {
  return fmt::format("{}\n{}\n{}", (s.defs | map([&](const StructDef &__24) { return repr(__24); }) | mk_string("\n"s)), repr(s.entry),
                     (s.functions | map([&](const Function &__25) { return repr(__25); }) | mk_string("\n"s)));
}
