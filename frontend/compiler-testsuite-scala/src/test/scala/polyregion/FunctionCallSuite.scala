...
class FunctionCallSuite extends BaseSuite {
...

  {
    object A {
      ...
      def double2(x: Double)                                 = A.x * x * 2.0
    ...
    // testCapture("module-const")(A.double(1.0))
    // testCapture("module-const")(A.double2(1f))
    // testCapture("module-const")(A.double(A.x))
...
//      testCapture("module-inlinemix-nest")(timesInlineMix(timesInlineMix(timesInlineMix(x, 2f), 3f), 4f))
  ...
  testCapture("module-const")(double(1d +M.x))
