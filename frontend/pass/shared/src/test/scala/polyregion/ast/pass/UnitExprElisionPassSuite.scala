...
class UnitExprElisionPassSuite extends munit.FunSuite {
  ...
  test("pure Unit var binding is elided") {
    val e = entry(body =
      List(p.Stmt.Var(named("u", p.Type.Unit0), Some(p.Expr.Unit0Const)), p.Stmt.Return(p.Expr.Unit0Const))
    )
  ...
  test("Unit-typed Invoke is preserved (potential side effect)") {
    val invoke = p.Expr.Invoke(sym("doIt"), Nil, None, Nil, Nil, p.Type.Unit0)
    val e   = entry(body = List(p.Stmt.Var(named("u", p.Type.Unit0), Some(invoke)), p.Stmt.Return(p.Expr.Unit0Const)))
    val out = UnitExprElisionPass(program(e), NoopLog)
