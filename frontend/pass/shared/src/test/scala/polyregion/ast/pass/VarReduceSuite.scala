...
class VarReduceSuite extends munit.FunSuite {
  ...
  test("trivial alias is collapsed: var b = a; uses(b) -> uses(a)") {
    ...
    val out = VarReduce(program(entry(body = body, args = Nil).copy(rtn = p.Type.IntS32)), NoopLog)
  ...
  test("running pass twice is idempotent") {
    ...
    val once  = VarReduce(program(entry(body = body).copy(rtn = p.Type.IntS32)), NoopLog)
    val twice = VarReduce(once, NoopLog)
