...
class UnitExprElisionSuite extends munit.FunSuite {
  ...
  test("pure Unit var binding is elided") {
    val e = entry(body =
      List(
        p.Stmt.Var(named("u", p.Type.Unit0), Some(p.Expr.Alias(p.Term.Unit0Const))),
        p.Stmt.Return(p.Expr.Alias(p.Term.Unit0Const))
      )
    )
    val out  = UnitExprElision(program(e), NoopLog)
  ...
  test("Unit-typed Invoke is preserved (potential side effect)") {
    ...
    val e = entry(body =
      List(p.Stmt.Var(named("u", p.Type.Unit0), Some(invoke)), p.Stmt.Return(p.Expr.Alias(p.Term.Unit0Const)))
    )
    val out     = UnitExprElision(program(e), NoopLog)
