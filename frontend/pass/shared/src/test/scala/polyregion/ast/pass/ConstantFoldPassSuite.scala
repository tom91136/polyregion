...
class ConstantFoldPassSuite extends munit.FunSuite {

  private def fold(body: List[p.Stmt], rtn: p.Type = p.Type.Unit0): List[p.Stmt] =
    ConstantFoldPass(program(entry(body = body).copy(rtn = rtn)), NoopLog).entry.body
  ...
  test("running pass twice is idempotent") {
    ...
    val once  = ConstantFoldPass(program(entry(body = body).copy(rtn = p.Type.IntS32)), NoopLog)
    val twice = ConstantFoldPass(once, NoopLog)
