...
class SubgroupLowerSuite extends munit.FunSuite {
  ...
  test("lowers every shuffle form and keeps subgroup barriers and unrelated operations visible") {
  ...
  test("keeps nested shuffle synchronization inside its control-flow scope") {
    ...
    assert(loopBody.count {
      case p.Stmt.Var(_, Some(p.Expr.SpecOp(_: p.Spec.GpuSubgroupBarrier)), _) => true
      case _                                                                   => false
    } >= 2)
  ...
  test("sizes vote per subgroup and ballot per lane without sharing storage") {
    ...
    assertEquals(
      types,
      Set[p.Type](p.Type.Arr(p.Type.Bool1, 32, p.Type.Space.Local), p.Type.Arr(p.Type.Bool1, 256, p.Type.Space.Local))
    )
    ...
    assert(out.entry.collectAll[p.Expr].exists {
      case p.Expr.IntrOp(p.Intr.BSR(_, p.Term.IntU32Const(3), p.Type.IntU32)) => true
      case _                                                                  => false
    })
  }
