...
class LibraryDeclarationSuite extends munit.FunSuite {
  ...
  test("a concrete call binds type variables and a callable by structure") {
    ...
    assertEquals(
      ...
      Right(LibraryBinding.Binding(Map("T" -> p.Type.Float32), Map(3 -> lambda.name)))
    )
  }
  ...
  test("callback overload selection incorporates later argument inference") {
    ...
    assertEquals(
      ...
      Right(LibraryBinding.Binding(Map("T" -> p.Type.Float32), Map(0 -> name)))
    )
  }
  ...
  test("explicit public types make callable overload checks independent") {
    ...
    assertEquals(
      ...
      Right(LibraryBinding.Binding(Map("T" -> p.Type.Float32), Map(0 -> first, 1 -> last)))
    )
  }
  ...
  test("a boundary declaration yields a structural argument plan") {
    import LibraryBinding.ArgumentKind
  ...
  test("binding respects callable type-variable scope and alpha-renaming") {
    ...
    assertEquals(
      decl.bind(call, Nil),
      Right(LibraryBinding.Binding(Map("T" -> p.Type.Float32), Map.empty))
    )
  }
  ...
  test("binding resolves consistent chained type substitutions") {
    ...
    assertEquals(
      decl.bind(call, Nil),
      Right(LibraryBinding.Binding(Map("T" -> p.Type.Float32, "U" -> p.Type.Float32), Map.empty))
    )
  }
  ...
  test("binding resolves a callable returned by reference") {
    ...
    assertEquals(decl.bind(call, List(callable)), Right(LibraryBinding.Binding(Map.empty, Map.empty)))
  }
  ...
  test("receiver extents classify referenced arguments") {
    ...
    assertEquals(
      ...
      Right(List(LibraryBinding.ArgumentKind.ExtentScalar))
    )
  }
