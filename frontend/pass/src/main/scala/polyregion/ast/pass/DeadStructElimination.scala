package polyregion.ast.pass

import polyregion.ast.Traversal.*
import polyregion.ast.{PolyAST as p, *, given}

import scala.annotation.tailrec

// drops StructDefs unreachable from the functions, following parents + member struct types
// examples:
//   used A{ b: B }, B, C; fn f(a: A)  ->  A, B   (C dropped, A reaches B via member)
//   used D extends E; fn f(d: D)       ->  D, E   (E kept via parent)
object DeadStructElimination extends ProgramPass {

  override def apply(program: p.Program, log: Log): p.Program = {
    def functionStructReferences(value: p.Function): Set[p.Sym] =
      value.collectWhere[p.Type] { case struct: p.Type.Struct => struct.name }.toSet

    def definitionStructReferences(value: p.StructDef): Set[p.Sym] =
      value.collectWhere[p.Type] { case struct: p.Type.Struct => struct.name }.toSet

    val roots: Set[p.Sym] =
      (program.entry.toList ::: program.functions)
        .flatMap(functionStructReferences)
        .toSet

    val byName = program.defs.map(d => d.name -> d).toMap

    @tailrec def reach(frontier: Set[p.Sym], live: Set[p.Sym]): Set[p.Sym] = {
      val next = frontier.flatMap { s =>
        byName.get(s).toList.flatMap { d =>
          definitionStructReferences(d)
        }
      } -- live
      if (next.isEmpty) live else reach(next, live ++ next)
    }

    val live = reach(roots, roots)
    if (log.enabled) {
      log.info("kept", live.toSeq.map(_.repr).sorted*)
      log.info("dropped", program.defs.map(_.name).filterNot(live.contains).map(_.repr).sorted*)
    }
    program.copy(defs = program.defs.filter(d => live.contains(d.name)))
  }

}
