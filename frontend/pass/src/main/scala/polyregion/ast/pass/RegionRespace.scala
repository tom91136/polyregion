    ...
    if (respace.isEmpty) (f, 0)
      ...
      val rooted = f
        ...
        .modifyAll[p.Stmt] {
          ...
          // Generic C++ aggregate fields have a fixed declared pointer space, but an inlined constructor may
          // store a pointer with proven Local/Constant provenance into one of those fields. Keep the stepped
          // assignment internally well-typed; later loads are respaced from the tracked field slot.
          case p.Stmt.Mut(lhs @ p.Term.Select(_, steps, t), e) if steps.nonEmpty =>
