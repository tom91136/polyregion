...
object compiletime {
  ...
  private def replaceTypes(mirrorToSourceTable: Map[p.Sym, p.Sym])(t: p.Type) = t match {
    case p.Type.Struct(sym, ref, tpeVars, args, parents) =>
      p.Type.Struct(mirrorToSourceTable.getOrElse(sym, sym), ref, tpeVars, args, parents) match {
        case p.Type.Struct(Symbols.ArrayMirror, ref, _, x :: Nil, _) =>
          ...
          rewrittenMirror =
            fn.copy(
              ...
              moduleCaptures = fn.moduleCaptures.filter(_.named.tpe match {
                case p.Type.Struct(sym, _, _, _, _) => sym != p.Sym(IntrinsicName)
                case _                              => true
              })
