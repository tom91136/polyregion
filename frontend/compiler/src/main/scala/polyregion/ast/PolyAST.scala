...
object PolyAST {
  ...
  enum Type(val kind: Type.Kind) derives MsgPack.Codec {
    ...
    case Struct(name: String, members: List[Named])                                 extends Type(Type.Kind.Ref)
  ...
  case class Signature(args: List[Type], rtn: Type) derives MsgPack.Codec
  ...
  case class Program(functions: List[Function]) derives MsgPack.Codec
